...
$controller = ($data == [""] || !$data) ? 'MainController' : ucfirst(array_shift($data)) . 'Controller';//если пустой контроллер, тогда переводим на дефолтный класс
$action = ($data == [""] || !$data) ? 'index' : array_shift($data);//пустой метод, переводим на дефолтный метод, дефолтный метод должен быть во всех классах
...
if ($controller === 'AdminController' && Session::get('user', ['login' => ''])['login'] !== 'admin') {
    //если контроллер Admin а пользователь не admin ошибка доступа 403
    die('404 - не завезли таких страниц ;)');
}
$controllerObj = new $controller();//создадим экземпляв класса контроллер
if (!method_exists($controllerObj, $action)) {//если метод не существует
